...
class SimpleErrorCounter:
    ...
    def record_error(self, agent_type: str):
        """记录错误"""
        if agent_type not in self._error_counts:
            self._error_counts[agent_type] = 0
        self._error_counts[agent_type] += 1
    
    def get_error_count(self, agent_type: str) -> int:
        """获取错误计数"""
        return self._error_counts.get(agent_type, 0)


# 创建全局错误计数器实例
simple_error_counter = SimpleErrorCounter()
            ...
            while retries < max_retries:
                try:
                    ...
                    if retries >= max_retries:
                        ...
                        if agent_type:
                            simple_error_counter.record_error(agent_type)
                        raise
                    ...
                    logger.error(f"函数 {func.__name__} 执行失败，非重试异常: {e}")
                    if agent_type:
                        simple_error_counter.record_error(agent_type)
                    raise
...
class ErrorContext:
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        if exc_val:
            ...
            # 记录响应时间（使用简单监控）
            from core.monitoring_decorator import simple_monitoring_system
            if self.agent_type:
                simple_monitoring_system.record_response_time(self.agent_type, execution_time)
